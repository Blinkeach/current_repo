...
const AdminPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (params?.tab && !['products', 'orders', 'customers', 'analytics', 'settings', 'navbar', 'hero-slides', 'messages', 'help', 'support', 'livechat', 'returns'].includes(params.tab)) {
      navigate('/admin');
    }
  }, [params, navigate]);
  ...
  const getTabTitle = () => {
    switch (currentTab) {
      ...
      case 'navbar': return 'Website Management';
    ...
    switch (currentTab) {
      ...
      case 'navbar':
        return <div className="space-y-6">
          ...
          <p className="text-gray-600">Manage your website's navigation bar and homepage carousel settings</p>
          <NavbarSettings />
        </div>;
      case 'messages':
        return <div className="space-y-6">
