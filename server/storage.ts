import { 
  ...
  referrals, referralRewards, supportRequests, wishlistItems, navbarSettings, carouselImages,
  productVariants, heroSlides,
  User, InsertUser, 
...
export interface IStorage {
  ...
  updateOrderInvoice(id: number, invoiceUrl: string | null): Promise<Order | undefined>;
  ...
  getCarouselImages(bannerType?: string): Promise<CarouselImage[]>;
  createCarouselImage(image: InsertCarouselImage): Promise<CarouselImage>;
  updateCarouselImage(id: number, imageData: Partial<CarouselImage>): Promise<CarouselImage | undefined>;
  deleteCarouselImage(id: number): Promise<boolean>;
  
  // Hero Slides methods
  getHeroSlides(): Promise<HeroSlide[]>;
  getHeroSlideById(id: number): Promise<HeroSlide | undefined>;
  createHeroSlide(slide: InsertHeroSlide): Promise<HeroSlide>;
  updateHeroSlide(id: number, slideData: Partial<HeroSlide>): Promise<HeroSlide | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async createProduct(insertProduct: InsertProduct): Promise<Product> {
    ...
    const product: Product = { 
      ...
      highlights: insertProduct.highlights ?? null,
      specifications: insertProduct.specifications ?? null,
      hasVariants: insertProduct.hasVariants ?? false,
      model3d: insertProduct.model3d ?? null,
      createdAt: now,
      updatedAt: now
    };
  ...
  async createOrder(insertOrder: InsertOrder): Promise<Order> {
    ...
    const order: Order = { 
      ...
      userEmail: insertOrder.userEmail ?? null,
      userPhone: insertOrder.userPhone ?? null,
      createdAt: now,
      updatedAt: now 
    };
  ...
  async updateOrderInvoice(id: number, invoiceUrl: string | null): Promise<Order | undefined> {
    const order = this.orders.get(id);
    if (!order) return undefined;
  ...
  async addOrderItem(insertOrderItem: InsertOrderItem): Promise<OrderItem> {
    ...
    const orderItem: OrderItem = { 
      ...insertOrderItem, 
      id, 
      selectedColor: insertOrderItem.selectedColor ?? null,
      selectedSize: insertOrderItem.selectedSize ?? null,
      hsnCode: insertOrderItem.hsnCode ?? null,
  ...
  // Missing methods for MemStorage interface compliance
  ...
  async getSupportRequestById(id: number): Promise<SupportRequest | undefined> {
    return undefined;
  }

  async getSupportRequestsByStatus(status: string): Promise<SupportRequest[]> {
    return [];
  }

  async createSupportRequest(supportRequest: InsertSupportRequest): Promise<SupportRequest> {
    const now = new Date();
    return { 
      id: 1, 
      ...supportRequest, 
      status: "open",
      email: supportRequest.email ?? null,
      phone: supportRequest.phone ?? null,
      message: supportRequest.message ?? null,
      preferredLanguage: supportRequest.preferredLanguage ?? null,
      notes: supportRequest.notes ?? null,
      createdAt: now, 
      updatedAt: now 
    };
  }
  ...
  async getCarouselImages(bannerType?: string): Promise<CarouselImage[]> {
    return [];
  }
  ...
  async createCarouselImage(image: InsertCarouselImage): Promise<CarouselImage> {
    ...
    return { 
      ...
      buttonLink: image.buttonLink ?? null,
      createdAt: now, 
      updatedAt: now 
    };
  }

  async updateCarouselImage(id: number, imageData: Partial<CarouselImage>): Promise<CarouselImage | undefined> {
    return undefined;
  }
  ...
  async getHeroSlideById(id: number): Promise<HeroSlide | undefined> {
    return undefined;
  }
  ...
  async updateHeroSlide(id: number, slideData: Partial<HeroSlide>): Promise<HeroSlide | undefined> {
    return undefined;
  }
...
export class DatabaseStorage implements IStorage {
  ...
  async getTopSellingProducts(limit: number): Promise<(Product & { totalSold: number })[]> {
    try {
      ...
      .groupBy(products.id)
      .orderBy(sql`COALESCE(SUM(${orderItems.quantity}), 0) DESC`, desc(products.rating))
      .limit(limit);
      
      return result;
    } catch (error) {
      console.error("Error in getTopSellingProducts with sales data, falling back to products by rating:", error);
      
      // Fallback: Get products sorted by rating if complex query fails
      const fallbackResult = await db.select().from(products)
        .orderBy(desc(products.rating), desc(products.reviewCount))
        .limit(limit);
      
      return fallbackResult.map(product => ({
        ...product,
        totalSold: 0
      }));
    }
  ...
  async getCarouselImages(bannerType?: string): Promise<CarouselImage[]> {
    let whereConditions = [eq(carouselImages.isActive, true)];
    
    // Filter by banner type if specified
    if (bannerType) {
      whereConditions.push(eq(carouselImages.bannerType, bannerType));
    }
